...
const TVDetails2 = () => {
    ...
    const [activeTab, setActiveTab] = useState("ontheair");
    ...
    const fetchData = async () => {
        try{
          const response = await fetchApiData("/tv/on_the_air")
        ...
        const handleClick = (id) => {
            navigate(`/ontheair/${id}`);
         };

  return (
    <div className='bg-black flex flex-col justify-between mt-0 md:pr-64 my-0 items-center md:items-start'>
        <h1 className='text-white md:text-3xl text-2xl font-semibold  cursor-pointer md:ml-60 ml-0 md:mt-8 mt-4'><span className="font-bold text-3xl  text-pink-700">|</span>ON THE AIR</h1>
         <Link
          to='/ontheair'
          className={`text-white md:text-3xl text-xs hover:text-pink-800 cursor-pointer md:ml-60 ml-9 ${
            activeTab === 'ontheair' ? 'font-bold text-yellow-400' : ''
          }`}
          onClick={() => handleTabChange('ontheair')}
        >
        </Link>
